...
void filling_square(cairo_t *cr, double x1, double y1, double x2, double y2, double x3, double y3) {
    double step = 0.001;
    ...
    cairo_set_source_rgb(cr, 1, 0.4, 0.2);
    cairo_fill(cr);


    for (double t = 0.0; t <= 1.0; t += step) {

        double px1 = x1 + (x2 - x1) * t;
        double py1 = y1 + (y2 - y1) * t;

        double px2 = x1 + (x3 - x1) * t;
        double py2 = y1 + (y3 - y1) * t;
...
static void draw(GtkWidget *widget, cairo_t *cr, gpointer data) {
    ...
    filling_square(cr, x1, start_y1, x2, y2, x3, y3);
